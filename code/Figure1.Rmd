...
biomarker.res <- fread("~/Code/tissue_biomarker/rnaResultsPrism/biomarker_res/mrna_gene_drug_tissue_res_pharmacodb.csv")
...
toPlot <- test[Sig%in%0, as.list(prop.table(table(FWER_genes<0.05))), PSet]
...
toPlot <- biomarker.res[FWER_genes<=0.05, .N, .(Drug, Tissue)][order(N)]
...
```{r}
## TODO:: complete the drug indication table!
