...
drugsMoreThan100 <- metaRes[FWER_genes < 0.05 & Tissue == "Lung", .N, Drug][N >= 20, Drug]

metaRes3 <- metaRes2[, multiDrugTable[N > 4 & Tissue == "Lung", unique(Gene)]]
metaRes3 <- metaRes3[rowSums(metaRes3 != 0) > 20, ]
...
pheatmap(metaRes3[rowSums(metaRes3!=0)>1,],
    clustering_distance_rows = "correlation", clustering_distance_cols = "correlation",
    clustering_method = "ward.D2", show_colnames = F, treeheight_col = 0
)
...
pdf("figures/lung_heatmap.pdf", height = 7, width = 5)
pheatmap(metaRes2,
    clustering_distance_rows = "correlation", clustering_distance_cols = "correlation",
    clustering_method = "ward.D2", show_colnames = F, treeheight_col = 0
)
...
median(apply(metaRes2, 1, \(x)(sum(x != 0))))
...
GeneAssignment <- data.frame(Gene = c(UpIn1Genes, UpIn2Genes), Cluster=c(rep(1,times=length(UpIn1Genes)),rep(2, times=length(UpIn2Genes))))

fwrite(GeneAssignment, file="LungClusteringGeneAssignment.csv")
